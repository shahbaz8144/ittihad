...
declare var $: any;
@Component({
  selector: 'app-gacfileupload',
  templateUrl: './gacfileupload.component.html',
  styleUrls: ['./gacfileupload.component.css']
})

export class GacfileuploadComponent implements OnInit {
...
this.route.queryParams.subscribe((params:any)=>{
  ...
  updateSelectedValues(UserId: number, event: any) {
    ...
    this.ObjShareUserList.forEach(element => {
      ...
      // console.log(this.SelectedUsers ,"add share users");
    });
  ...
  GetDropdowns() {
    ...
    this.inboxService.ShareUserList(this._obj1)
      .subscribe(data => {
        ...
        //console.log(this.ObjUserList, "ObjUserList");
        ...
        this.ObjShareUserList.forEach(_user => {
          if (_user.UserId === this._ReportingUserId) {
            ...
            // this.updateSelectedValues(_user.ContactName, { checked: true });
            // this.updateSelectedArray(this.selectedValues); // Update _selectedValues before assigning

            this.SelectedUsers.push(_user);
            this.SelectedReportingUserName = _user.ContactName;
            // if (this._selectedValues.length > 0) {
            //   this.SelectedReportingUserName = this._selectedValues[0].ContactName;
            // } else {
            //   this.SelectedReportingUserName = ''; // or any other default value
            // }
          }
        });
      });
  ...
  selectFile(event: any) {
    // let uniqueIdCounter = Date.now(); // Initialize with current timestamp to avoid any initial duplicates
    ...
    //     this._GacAttachmentFileuplod = [...this._GacAttachmentFileuplod, {
    //       UniqueId: d,
    //       FileName: event.target.files[index].name,
    //       Size: event.target.files[index].size,
    //       Files: event.target.files[index],
    //       Ismain: false,
    //       Thumbnail: '' // Initialize with an empty string or default value
    //     }];



        
    //     // console.log(this._GacAttachmentFileuplod, "File Upload");
    //     // Initialize thumbnail preview array
    ...
    if (files.length > 0) {
      for (let index = 0; index < files.length; index++) {
        ...
        if (!existingFile) {
          ...
          this._GacAttachmentFileuplod.push({
            UniqueId: uniqueId,
            FileName: file.name,
            Size: file.size,
            Files: file,
    ...
    console.log(this._GacAttachmentFileuplod, "Files");
  ...
  WorkFlowDetails(WorkFlowId: number) {
    ...
    this.inboxService.GetWorkFlowDetailsAPI(this._obj1)
      .subscribe(data => {
       ...
       console.log(this.ObjShareUserList , "Share User"); 
